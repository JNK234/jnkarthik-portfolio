---
title: "Deeper Alignment Made a Worse Model"
slug: "sycophancy-recovery-ipo"
date: 2026-04-12
description: "IPO achieves the deepest representational change of any alignment method — probe transfer 0.365, layer-3 peak, fully distributed signal — but the worst behavioral outcome. The reference model hypothesis breaks. Part 3 of a series on sycophancy recovery."
summary: "IPO restructured the model more deeply than SimPO (probe transfer 0.365 vs 0.429) but performed worse behaviorally (0.281 vs 0.176). The reference model doesn't limit intervention depth — the loss shape does. A 2x2 framework replaces the 1D hypothesis."
tags: ["AI Safety", "Alignment", "Mechanistic Interpretability", "LLMs"]
categories: ["technical"]
showTableOfContents: true
---

*Part 3 of a series on sycophancy recovery — comparing alignment techniques from the inside out, using behavioral evaluation and mechanistic interpretability. [Part 1: DPO]({{< ref "/posts/sycophancy-recovery-dpo" >}}) · [Part 2: SimPO]({{< ref "/posts/sycophancy-recovery-simpo" >}})*
...
![IPO recovers sycophancy to near-DPO levels — but with steeper capability damage. Aggregate sycophancy 0.281 vs DPO's 0.268 and SimPO's 0.176. Feedback sycophancy notably higher at 0.170.](fig1_behavioral_comparison.png)
...
![SFT sycophancy probe fails on IPO — transfer 0.365, the lowest of all methods. DPO transfer remains significant (p=0.005). SimPO and IPO transfers are not significant after multiple-comparison correction.](fig2_probe_transfer.png)
...
![IPO peaks at layer 3 while all other models peak between layers 17-22. The orange IPO curve shows a clear early spike — sycophancy-relevant processing moved from late layers to one of the earliest.](fig3_layer_auroc_curves.png)
...
![After removing the primary sycophancy direction, fresh probes recover to 0.73-0.81. IPO is unique: retrained AUROC (0.814) equals the original (0.811). Removing the top direction has zero effect — the signal is fully distributed.](fig4_ablation_comparison.png)
